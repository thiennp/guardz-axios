import { isString, isNumber } from 'guardz';
import {
  isAxiosResponse,
...
describe('Axios Response Guards', () => {
  ...
  describe('isSuccessResponse', () => {
    ...
    it('should return false for non-2xx status codes', () => {
      expect(isSuccessResponse(createMockResponse({ status: 300 }))).toBe(false);
      expect(isSuccessResponse(createMockResponse({ status: 400 }))).toBe(false);
      expect(isSuccessResponse(createMockResponse({ status: 500 }))).toBe(false);
    });
  });
  ...
  describe('isSuccessResponseWithData', () => {
    it('should validate successful response with correct data type', () => {
      ...
      const userGuard = (value: unknown): value is { id: number; name: string } => {
  ...
  describe('isClientErrorResponse', () => {
    it('should return true for 4xx status codes', () => {
      expect(isClientErrorResponse(createMockResponse({ status: 400 }))).toBe(true);
      expect(isClientErrorResponse(createMockResponse({ status: 404 }))).toBe(true);
      expect(isClientErrorResponse(createMockResponse({ status: 499 }))).toBe(true);
    });

    it('should return false for non-4xx status codes', () => {
      expect(isClientErrorResponse(createMockResponse({ status: 200 }))).toBe(false);
      expect(isClientErrorResponse(createMockResponse({ status: 500 }))).toBe(false);
    });
  });

  describe('isServerErrorResponse', () => {
    it('should return true for 5xx status codes', () => {
      expect(isServerErrorResponse(createMockResponse({ status: 500 }))).toBe(true);
      expect(isServerErrorResponse(createMockResponse({ status: 503 }))).toBe(true);
      expect(isServerErrorResponse(createMockResponse({ status: 599 }))).toBe(true);
    });

    it('should return false for non-5xx status codes', () => {
      expect(isServerErrorResponse(createMockResponse({ status: 200 }))).toBe(false);
      expect(isServerErrorResponse(createMockResponse({ status: 400 }))).toBe(false);
    });
  });
  ...
  describe('isCreatedResponse', () => {
    ...
    it('should return false for non-201 status codes', () => {
      expect(isCreatedResponse(createMockResponse({ status: 200 }))).toBe(false);
      expect(isCreatedResponse(createMockResponse({ status: 202 }))).toBe(false);
    });
  });
